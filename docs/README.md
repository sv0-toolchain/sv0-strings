...
| [`host-capability-abi-scoping.md`](host-capability-abi-scoping.md) | SS-U12/UP-015/OQ-005: the design decision (POSIX-only on both backends, generation-free capability handles; named-locale/CLDR-table support deferred to a separate future slice) that `strings_locale::open`'s real implementation will follow |
