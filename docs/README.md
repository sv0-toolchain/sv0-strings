...
| [`serialized-comparison.md`](serialized-comparison.md) | `check_serialized_comparison.py` | package-owned `{ok\|err\|skip, bytes}` record per fixture+backend, cross-checked independently of exit-code comparison; fixture-ID digest; injected-mismatch self-test |
...
| [`locale-and-host-capabilities.md`](locale-and-host-capabilities.md) | `strcoll`/`strxfrm`/`strerror` → locale-open lifecycle → `_l` adapters → host error/signal messages, one narrative across SS-150/167/168/169. SS-U12 (2026-09-18) landed real POSIX-locale collation for §1–§3; §4 (host error/signal message text) stays a capability stub pending the separate FFI prerequisite |
| [`host-capability-abi-scoping.md`](host-capability-abi-scoping.md) | SS-U12/UP-015/OQ-005: IMPLEMENTED 2026-09-18 (Option B, POSIX-only on both backends) — the design decision `strings_locale`'s real implementation follows, plus two toolchain-forced representation adjustments found while landing it (capability id as raw `usize`, not a wrapping struct; renamed public functions to dodge a cross-module name-collision bug) |
